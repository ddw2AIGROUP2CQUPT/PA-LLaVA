...
class EvaluateChatHook(Hook):
    ...
    def __init__(self,
        ...
        self.anyres_image = anyres_image
        ...
        for sample_image, sample_input in zip(self.evaluation_images,
            ...
            if self.pad_image_to_square:
                ...
                image = self.image_processor.preprocess(
                    image, return_tensors='pt')['pixel_values'][0].unsqueeze(0)
            elif self.anyres_image:
                image = process_anyres_image(sample_image, self.image_processor, self.image_grid_pinpoints)
            else:
                image = self.image_processor.preprocess(
                    sample_image, return_tensors='pt')['pixel_values'][0].unsqueeze(0)
            ...
            with torch.no_grad():
                visual_outputs = model.visual_encoder(
                    image[0].unsqueeze(0).to(model.visual_encoder.dtype),
                    output_hidden_states=True)
            
            if self.projector_type == 'mlp':
                pixel_values = model.projector(
                    visual_outputs.hidden_states[model.visual_select_layer][:, 1:])
            elif self.projector_type == 'qformer_mlp':
                ori_pixel_embeds = visual_outputs.hidden_states[model.visual_select_layer][0, 1:].unsqueeze(0)
                dim = ori_pixel_embeds.shape[-1]
                patch_pixel_embeds = visual_outputs.hidden_states[model.visual_select_layer][1:, 1:].reshape(-1, dim).unsqueeze(0)
                pixel_values = model.projector(ori_pixel_embeds, patch_pixel_embeds)
            else:

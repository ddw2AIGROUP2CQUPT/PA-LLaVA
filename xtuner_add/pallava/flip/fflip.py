...
def init_tokenizer():
    ...
    tokenizer.add_special_tokens({'additional_special_tokens':['[ENC]','[DEC]']})       
    tokenizer.enc_token_id = tokenizer.additional_special_tokens_ids[0]  
    tokenizer.dec_token_id = tokenizer.additional_special_tokens_ids[1] 
    return tokenizer
...
class Attention(nn.Module):
    ...
    def forward(self, x):  # 768 12 12
        attn_mask = self.net(x)  # 1 12 12 
        attn_mask = attn_mask.view(attn_mask.size(0), -1) 
        attn_mask = nn.Softmax(dim=1)(attn_mask) # 转换为概率
        attn_mask = attn_mask.view(attn_mask.size(0), 1, x.size(2), x.size(3))  #1 12 12
        x = x + (x * attn_mask)

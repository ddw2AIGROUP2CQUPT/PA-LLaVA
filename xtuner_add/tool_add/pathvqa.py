...
class EvalDataset(Dataset):
    ...
    def __init__(self, data_path):
        ...
        self.data = data
        self.img_dir = "path to pathvqa/images/test/"
    ...
    def pathvqa_eval(self, respath, recall=False, toknizer=None):

        vqaEval = VQAEval(self.data_path,respath,n=2)
        vqaEval.evaluate(recall,toknizer)
...
def main():
    ...
    results_json_path = osp.join(save_dir, 'pathvqa_result.json')
    ...
    if get_rank() == 0:
        with open(results_json_path, 'w') as f:
            json.dump(results, f)
        # dataset.pathvqa_eval(results_json_path,True,tokenizer)
        dataset.pathvqa_eval(results_json_path,False)
